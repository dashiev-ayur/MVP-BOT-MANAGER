...
**Сделать** (можно дробить на UI-6.1 / 6.2 / 6.3 по запросу пользователя)

#### UI-6.1 — Runtimes + карточка ноды

- [x] `/runtimes` — таблица + lease warning; связь фильтром ботов  
- [x] `/nodes/:id` — боты и runtimes на ноде  

**Критерии tester (6.1)**

- [x] build PASS  
- [x] lease warning; `/bots?runtime_id=` фильтрует  
- [x] карточка ноды + «не найдена»  

**Ручная проверка (6.1)**

1. Runtimes видны, клик ведёт к ботам с фильтром.  
2. С `/nodes` открыть карточку; неизвестный id → «не найдена».
...
#### UI-6.3 — Toasts + авто-poll + README

- [x] Toasts на успех/ошибку команд  
- [x] Авто-poll 3–10s на списках/карточке + «обновлено N с назад»  
- [x] Фильтр событий по типу (если просто)  
- [x] Обновить `web/README.md` и при необходимости корневой README  

**Критерии tester (6.3)**

- [x] build PASS  
- [x] poll не устраивает полный unmount-мигание  
- [x] toasts на команды; README обновлён  

**Ручная проверка (6.3)**

1. Списки ~15s — данные обновляются без мигания; stale-текст.  
2. Start/Stop/Migrate/Create/Patch → toast.  
3. Фильтр типа events на карточке.

**Принято вами (UI-6.3):** [x] (2026-07-19)

**Не делать:** dark theme; e2e playwright suite; embed в control-api; handoff wizard.

**Принято вами (весь UI-6):** [x] (2026-07-19)
...
- [x] Все блоки UI-0a … UI-5 с **Принято вами**; UI-6 — если входил в договорённый scope P0+P1  
- [x] Критерии [frontend.md](./frontend.md) §13.1 закрыты  
- [x] Пройден ручной сценарий §13.2  
- [x] Сводка в IMPLEMENTATION_PLAN: Phase UI → `✅ принята`  

**Принято вами (вся Phase UI):** [x] (2026-07-19)
